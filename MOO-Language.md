...
40. GET_DIGIT(original: int, place: int)
    ...
    ``` python
    1) 목적/변화: 숫자에서 원하는 자리수 추출
    2) 인수값: original -> 원본 숫자 / place -> n번째 자리수 
    3) 결과값: 숫자
    4) 예시: 
        R0: GET_DIGIT(149, 2) --> 결과
    ```
    
41. CHANGE_DIGIT(original: int, place: int, digit: int)
    
    ``` python
    1) 목적/변화: 자리수
    2) 인수값: original -> 원본 숫자 / place -> 교체하고자 하는 자리수 / digit -> 교체 당하는 자리수
    3) 결과값: 자리수가 교체된 숫자
    ```
    
42. GET_ITEM(ls1: List[Any], item: Union[str, int, float]) -> int

    ``` python
    1) 목적/변화: 리스트 속에 있는 원소의 인덱스를 뽑는다
    2) 인수값: lst -> 리스트 / item -> 인덱스를 알고자하는 원소
    3) 결과값: 원소의 인덱스
    4) 예시: 준석이가 선두로 재명, 준표 이 순서대로 뒤따라 달리고 있다. 준석은 몇 등인가?
        (LIST 구성됐다고 가정..)
        R0: GET_ITEM('준석')
    ```
    
43. LIST_SORT(lst: List[Union[int, float]]) -> List[Union[int, float]]

    ``` python
    1) 목적/변화: 리스트를 ascending order로 정렬 (min -> max)
    2) 인수값: 정렬하고자 하는 리스트
    3) 결과값: 정렬된 리스트
    ```
    
44. CIR_AREA(radius: Union[int, float]) -> float

    ``` python
    1) 목적/변화: 원의 넓이를 구한다
    2) 인수값: 원의 반지름(정수 혹은 실수)
    3) 결과값: 원의 넓이
    ```
    
45. CIRCUM(radius: Union[int, float]) -> float
...
46. SPHERE_SURFACE(radius: Union[int, float]) -> float
    ``` python
    1) 목적/변화: 구의 겉넓이를 구한다
    2) 인수값: 구의 반지름(정수 혹은 실수)
    3) 결과값: 구의 겉넓이
    ```
47. SPHERE_VOLUME(radius: Union[int, float]) -> float
...
48. TRI_AREA(a: Union[int, float], b: Union[int, float], c: Union[int, float]) -> Union[int, float]

    ``` python
    1) 목적/변화: 헤론의 공식을 이용하여 삼각형의 넓이
    2) 인수값: 삼각형의 세 변의 길이 (정수 혹은 실수)
    3) 결과값: 삼각형의 넓이
    4) 예시: 삼각형의 변의 길이가 각각 1, 2, 3일 때 넓이를 구하여라
        R0: TRI_AREA(1,2,3)
        R1: PRINT(R0)
    ```
    
49. POLY(polygon: str) -> int

    ``` python
    1) 목적/변화: 'n각형'을 입력하면 그에 맞는 각/변의 개수를 돌려준다.
    2) 인수값: n각형
    3) 결과값: 각/변의 개수
    4) 예시: 
        R0: POLY('삼각형') --> 결과: 3
